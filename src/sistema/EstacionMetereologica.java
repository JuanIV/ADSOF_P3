...
public class EstacionMetereologica implements IDocumento {
	...
	/** Valor por defecto del porcentaje de cambio considerado cambio brusco */
	...
	/**
	 * Porcentaje de diferencia a partir del cual se considera cambio brusco una
	 * medición.
	 */
	...
	private class SensorInstalado {
		 ...
		 * To string.
		 *
		 * @return String formateado imprimiendo la información completa de un sensor de
		 *         la estación
		 */
		@Override
		public String toString() {
			...
			if (!sensor.getHistorial().isEmpty())
				str.append(String.format(" -- MIN: %.2f MAX: %.2f AVG: %.2f", sensor.getMinimo(), sensor.getMaximo(),
						sensor.getMedia()));
			return str.toString();
		}
	...
	public EstacionMetereologica(String nombre, double longitud, double latitud, double cambioBrusco) {
		...
		this.setCambioBrusco(cambioBrusco);
	}
	...
	public EstacionMetereologica(String nombre, double longitud, double latitud) {
		...
		this.longitud = longitud;
		this.latitud = latitud;
		this.cambioBrusco = DF_SUDDEN_CHANGE_VAL;
	}
	...
	public List<Sensor<?>> getSensores() {
		...
		sensores.values().forEach(si -> resultado.add(si.sensor));
		return resultado;
	}
	...
	public <T extends Sensor<?>> List<T> getSensoresPorTipo(Class<T> tipo) {
		return sensores.values().stream().map(si -> si.sensor).filter(s -> tipo.isInstance(s)).map(s -> tipo.cast(s))
	...
	public void setCambioBrusco(double cambioBrusco) {
		/* Permitimos porcentajes mayores que 100 */
		if (cambioBrusco < 0)
			cambioBrusco = 0;
	...
	public LocalDateTime getFechaUltimaLectura() {
		if (sensores.isEmpty())
			return null;
		SensorInstalado min = sensores.values().toArray(new SensorInstalado[0])[0];
	...
	private void medir(SensorInstalado si) {
		if (alertas.containsKey(si.sensor)
				&& alertas.get(si.sensor).stream().anyMatch(a -> a instanceof SensorDescalibrado)) {
			return;
		}
		...
		if ((ultima != 0) && Math.abs((actual - ultima) / ultima) * 100.0 > cambioBrusco) {
			...
			/*
			 * Limpiamos las alertas de cambio brusco asociadas a este vector si no ha
			 * ocurrido ahora
			 */
	 ...
	 * @throws IncompatibleUnitsException  si el sensor tiene un tipo de unidad
	 *                                     desconocido
	 * @throws DuplicatedSensorIdException si ya existe un sensor con la misma id
	 */
	public void anadirSensor(Unidad ud, double offset, Estrategia estrategia, Conversor conv)
			throws IncompatibleUnitsException, DuplicatedSensorIdException {
		Sensor<?> sensor;
		...
		if (sensores.containsKey(si.sensor.getId())) {
			throw new DuplicatedSensorIdException("Ya existe un sensor con el mismo ID",
					sensores.get(si.sensor.getId()).sensor, si.sensor);
		}
	 ...
	 * @throws DuplicatedSensorIdException si ya existe un sensor con la misma id
	 */
	public void anadirSensor(Unidad ud, double offset, Estrategia estrategia)
			throws IncompatibleUnitsException, DuplicatedSensorIdException {
	 ...
	 * @throws DuplicatedSensorIdException si ya existe un sensor con la misma id
	 */
	public void anadirSensor(Unidad ud, double offset, Conversor conv)
			throws IncompatibleUnitsException, DuplicatedSensorIdException {
		anadirSensor(ud, offset, DF_STRATEGY(ud), conv);
	}
	...
	public void anadirSensor(Unidad ud, double offset) throws IncompatibleUnitsException, DuplicatedSensorIdException {
		anadirSensor(ud, offset, DF_STRATEGY(ud), Conversor.identidad(ud));
	}
	...
	public String[] getParrafos() {
		...
		parrafos.add("Ubicación: " + longitud + ", " + latitud);
